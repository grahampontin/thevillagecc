...
export const WicketScreen: React.FC<WicketScreenProps> = ({
  ...
  const waitingPlayersForWicket = localPlayers.filter(p => p.state === 'Waiting');
        ...
        <div className="max-w-lg mx-auto p-4">
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden space-y-0">
            <div className="flex items-center px-4 py-3 border-b border-gray-100">
              ...
              <select value={wicketBatterOutId ?? ''} onChange={e => setWicketBatterOutId(e.target.value ? Number(e.target.value) : null)} className="flex-1 text-sm text-gray-900 bg-transparent outline-none">
                {battingPlayersForWicket.map(p => <option key={p.playerId} value={p.playerId!}>{p.playerName}</option>)}
              </select>
            </div>
            <div className="flex items-center px-4 py-3 border-b border-gray-100">
              <label className="w-32 text-sm text-gray-600 flex-shrink-0">Dismissal</label>
              <select value={wicketDismissalCode} onChange={e => setWicketDismissalCode(e.target.value)} className="flex-1 text-sm text-gray-900 bg-transparent outline-none">
            ...
            {selectedDismissalMode?.hasFielder && (
              <div className="flex items-center px-4 py-3 border-b border-gray-100">
                ...
                <input type="text" placeholder="Add name..." value={wicketFielder} onChange={e => setWicketFielder(e.target.value)} className="flex-1 text-sm text-gray-900 bg-transparent outline-none" />
              </div>
            )}
            {selectedDismissalMode?.hasRuns && (
              <>
                <div className="flex items-center px-4 py-3 border-b border-gray-100">
                  ...
                  <input type="number" min={0} max={6} value={wicketRuns} onChange={e => setWicketRuns(e.target.value)} className="flex-1 text-sm text-gray-900 bg-transparent outline-none" />
                </div>
                {parseInt(wicketRuns, 10) > 0 && (
                  <div className="flex items-center px-4 py-3 border-b border-gray-100">
                    <label className="w-32 text-sm text-gray-600 flex-shrink-0">Runs/Extras?</label>
                    <select value={wicketRunsType} onChange={e => setWicketRunsType(e.target.value)} className="flex-1 text-sm text-gray-900 bg-transparent outline-none">
            ...
            {selectedDismissalMode?.hasCrossed && (
              <div className="flex items-center px-4 py-3 border-b border-gray-100">
                ...
                <select value={wicketBatsmenCrossed ? 'true' : 'false'} onChange={e => setWicketBatsmenCrossed(e.target.value === 'true')} className="flex-1 text-sm text-gray-900 bg-transparent outline-none">
            ...
            <div className="flex items-center px-4 py-3 border-b border-gray-100">
              ...
              <select value={wicketNextBatterInId} onChange={e => setWicketNextBatterInId(Number(e.target.value))} className="flex-1 text-sm text-gray-900 bg-transparent outline-none">
                <option value={-1}>{waitingPlayersForWicket.length === 0 ? 'Last wicket' : 'Select...'}</option>
                {waitingPlayersForWicket.map(p => <option key={p.playerId} value={p.playerId!}>{p.playerName}</option>)}
              </select>
            </div>
            <div className="flex items-start px-4 py-3">
              <span className="material-symbols-outlined text-gray-400 text-lg mr-3 mt-0.5">comment</span>
              <textarea placeholder="Ah nuts, talk us through it champ." value={wicketCommentary} onChange={e => setWicketCommentary(e.target.value)} rows={3} className="flex-1 text-sm text-gray-900 bg-transparent outline-none resize-none" />
            </div>
          </div>
        </div>
      </div>
