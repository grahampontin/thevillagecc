    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            switch (genericBallByBallCommand.command)
            {
                ...
                case "listPlayers":
                {
                    var players = Player.GetAll().Where(p => p.IsActive && p.ID > 0).OrderByDescending(p=>p.NumberOfMatchesPlayedThisSeason).ThenBy(p => p.FormalName).Select(PlayerV1.FromInternal).ToList();
