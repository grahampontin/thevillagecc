...
<tr><th align="left">Player</th><th>Payments Total</th><th>Games Played</th><th>Nets Attended</th><th>Balance</th></tr>
<%		tmpIndex=0
		...
		strQuery = "select count(*) as total from batting_scorecards a, matches b where a.match_id = b.match_id and match_date > 16/02/2006 and player_id ="&player_id
		...
		balance = payments-(5*RSgames("total"))-(2.5*RSnets("total"))
		if balance < 0 then
...
<tr <%if tmpindex/2 = round(tmpindex/2) Then%>bgcolor=#EEEEEE<%End If%>><td><%=RSplayer_id("player_name")%></td><td><%=cstr(payments)%></td><td><%=cstr(RSgames("total"))%></td><td><%=nets%></td><td><%=balance%></td></tr>
<%		
		end If
